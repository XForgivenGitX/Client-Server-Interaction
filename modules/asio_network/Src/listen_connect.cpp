...
void anet::connection::connection_request(socket_data_endpoint_ptr &&socketDataEndpoint, callback_func_t &&handler)
{
    ...
    utility::make_task_wrapped(std::move(handler))(std::move(socketDataEndpoint), error);
}
...
{
    boost::system::error_code error = socketDataEndpoint->connect();
    auto wrappedHandler = utility::make_task_wrapped(std::move(handler));
...
{
    utility::make_task_wrapped(std::move(functionUnwrapped_))(std::move(listener_), error);
}
